...
def test_embedder_encoding_and_l2_normalization() -> None:
    ...
    # Generate mock normalized output
    raw_vecs = np.random.randn(2, 384).astype(np.float32)
    norm_vecs = raw_vecs / np.linalg.norm(raw_vecs, axis=1, keepdims=True)
    mock_model.encode.return_value = norm_vecs

    with patch("src.embeddings.embedder.SentenceTransformer", return_value=mock_model):
